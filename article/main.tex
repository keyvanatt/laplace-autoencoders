...
Given the compact coefficient vectors, the surrogate task is reduced to predicting $2\,K\,k_{\mathrm{svd}}$ scalar targets from the three-dimensional parameter vector $\vmu$. A single \texttt{FreqSurrogate} network (shared trunk + per-frequency heads, same architecture as the AE surrogates) predicts the normalized coefficients for all frequencies jointly:
\begin{equation}
    \bm{c}^{(k,r)}, \bm{c}^{(k,i)} = g_{\bm{\theta}}(\vmu, k), \qquad k = 0,\dots,K-1.
\end{equation}
The surrogate is trained by minimizing a combined loss of normalized coefficient MSE and physical-field reconstruction MSE (after inverse Laplace transform), following the $\mathcal{L}_{\mathrm{int}} + \mathcal{L}_{\mathrm{rec}}$ decomposition introduced in~\cite{fresca2021}:
\begin{equation}
    \mathcal{L}_{\mathrm{SVD\text{-}surr}} = \alpha_{\mathrm{lat}}\sum_{k=0}^{K-1}\Bigl(\bigl\|g_{\bm{\theta}}^{(k,r)}(\vmu) - \bar{\bm{c}}^{(k,r)}\bigr\|_2^2 + \bigl\|g_{\bm{\theta}}^{(k,i)}(\vmu) - \bar{\bm{c}}^{(k,i)}\bigr\|_2^2\Bigr) + \alpha_{\mathrm{spat}}\,\bigl\|U_{\mathrm{rec}} - U\bigr\|_2^2,
\end{equation}
where $\bar{\bm{c}}^{(k,\cdot)}$ denotes the normalized coefficients and $U_{\mathrm{rec}}$ is the field reconstructed by back-projecting the predicted coefficients through the fixed bases and applying the inverse Laplace transform. The spatial bases $\bm{V}^{(k,\cdot)}$ remain \emph{fixed} throughout surrogate training: unlike the AE decoder, they are not fine-tuned.
...
In SLAE, the Laplace transform operates directly on the time-domain spatial pixels, transforming the sequence of 2D grids $U(t)$ into $K$ complex-valued frequency frames $\hat{U}(s_k)$. A \emph{frequency-conditioned} encoder (FiLM-modulated convolutional network) compresses each 2-channel (real and imaginary) frame independently into a real \emph{latent vector} $z(s_k) \in \R^{d_z}$:
\begin{equation}
...
Following the DL-ROM paradigm of~\cite{fresca2021}, the surrogate model $g_{\bm{\theta}}$ replaces the encoding steps by learning the direct mapping $\vmu \mapsto w_k$, where $w_k$ denotes the Laplace-domain latent at frequency $s_k$ that feeds the decoder: $w_k = z(s_k) \in \R^{d_z}$ for SLAE, and $w_k = \hat{z}(s_k) \in \C^{d_z}$ for LLAE. Two strategies for constructing the surrogate target $w_k$ are described below.
...
We first characterize the linear SVD baseline (Section~\ref{sec:svd_baseline}) by sweeping its three hyperparameters independently around a base configuration ($K=16$, $k_{\mathrm{svd}}=64$, $\gamma=0.01$): the number of Laplace frequencies $K \in \{8,16,32\}$, the number of SVD modes $k_{\mathrm{svd}} \in \{32,64,128\}$, and the damping parameter $\gamma \in \{0.0,0.001,0.01\}$. All evaluations are computed on the 1620 held-out test simulations using the pure SVD reconstruction (without a surrogate), i.e., projecting the true test fields onto the training-set bases.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{svd_sweep_gamma.png}
    \caption{Effect of the damping parameter $\gamma$ on SVD baseline reconstruction ($K=16$, $k_{\mathrm{svd}}=64$). Left: distribution of the relative $L^2$ error on the test set (violin plots; black line = median, dashed orange = mean). Right: per-frequency reconstruction error in the Laplace domain. A small positive $\gamma$ reduces aliasing artifacts at low frequencies without noticeably increasing error at high frequencies.}
    \label{fig:svd_sweep_gamma}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{svd_sweep_K.png}
    \caption{Effect of the number of Laplace frequencies $K$ on SVD baseline reconstruction ($k_{\mathrm{svd}}=64$, $\gamma=0.01$). Increasing $K$ beyond 16 yields only marginal improvement, indicating that the low-frequency modes already capture the dominant dynamics of the transient field.}
    \label{fig:svd_sweep_K}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{svd_sweep_ksvd.png}
    \caption{Effect of the number of SVD modes $k_{\mathrm{svd}}$ on SVD baseline reconstruction ($K=16$, $\gamma=0.01$). Left: physical-field error distribution. Right: per-frequency error in the Laplace domain. The error decreases monotonically with $k_{\mathrm{svd}}$ but saturates quickly, reflecting a hard reconstruction floor imposed by the linearity of the spatial basis.}
    \label{fig:svd_sweep_ksvd}
\end{figure}

The SVD reconstruction error is governed by the singular value decay of the Laplace-domain field matrices $\bm{U}^{(k)}$. The per-frequency breakdown (right panels of Figures~\ref{fig:svd_sweep_gamma} and~\ref{fig:svd_sweep_ksvd}) reveals that the linear basis is most accurate at low Laplace frequencies ($k=0$--$3$), where the spatial mode structure is smooth and low-rank, but degrades steadily at higher frequencies where the fields become more localized and the singular values decay more slowly. Increasing $k_{\mathrm{svd}}$ beyond 64 reduces this high-frequency error only marginally, indicating that the residual error is not a truncation artifact but an intrinsic limit of the linear subspace for this problem. Similarly, the choice of $K$ (Figure~\ref{fig:svd_sweep_K}) has little effect once the dominant transient content is captured by the first 16 frequencies: beyond $K=16$, additional modes add noise rather than information. The damping $\gamma$ (Figure~\ref{fig:svd_sweep_gamma}) is benign in the range $[0,0.01]$, with a small positive value slightly regularizing the low-frequency modes. These observations establish the linear SVD as a well-characterized reference: its residual error floor, set by the discarded singular values, is the target that the non-linear autoencoders of Section~\ref{sec:ae_perf} are designed to push below.
...
\begin{table}[H]
    ...
    \begin{tabular}{llcccrrrr}
        ...
        \midrule
        \multirow{7}{*}{LLAE}
          & 128 & 16 & 0.01  & 2.92 & 3.15 & 4.38  & $-4.12$ \\

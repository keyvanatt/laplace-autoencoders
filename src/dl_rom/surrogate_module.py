"""
surrogate_module.py — LightningModule pour l'entraînement end-to-end du surrogate DL-ROM (phase 2).

Batch : (theta_norm, U_norm) avec U_norm = (U - U_mean) / U_std  [domaine temporel].
Loss spatiale sur U_norm.
Supervision latente : encoder(U_norm) → z_true(t) (dans DLROMModel.forward).
"""
import numpy as np
import torch
...
class DLROMSurrogateLightningModule(pl.LightningModule):
    """Phase 2 : entraînement end-to-end surrogate DL-ROM θ→z(t)→U(t)."""
    ...
    def _build_model(self, dm):
        ...
        ae, latent_dim, dt, time_L = load_dlrom_from_ckpt(cfg_t.ae_ckpt, ds.N, Nt)
    ...
    def training_step(self, batch, batch_idx):
        ...
        U_rec, z_pred, z_true = self.model(th, u_true_norm)

        alpha_lat  = float(self.cfg.training.alpha_lat)
        alpha_spat = float(self.cfg.training.alpha_spat)
        loss, metrics = self.model.loss(u_true_norm, U_rec, z_pred, z_true,
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        m  = self.model
        checkpoint.update({
            'model_type':  'DLROMModel',
            'Nt':          m.Nt,
            't_stride':    int(getattr(ds, 't_stride', 1)),
            'N':           ds.N,
            'theta_dim':   ds.theta_dim,

...
The surrogate model $g_{\bm{\theta}}$ replaces the encoding steps by learning the direct mapping $\vmu \mapsto w_k$, where $w_k$ denotes the Laplace-domain latent at frequency $s_k$ that feeds the frozen decoder: $w_k = z(s_k) \in \R^{d_z}$ for SLAE, and $w_k = \hat{z}(s_k) \in \C^{d_z}$ for LLAE. Two strategies for constructing the surrogate target $w_k$ are described below.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \label{fig:llae_surr}
    \end{subfigure}
    \caption{Direct embedding prediction. The FreqSurrogate trunk MLP (4 layers, hidden dim 512) maps $\vmu$ to frequency-indexed latents, decoded by the frozen AE decoder. Left: SLAE predicts real latents $z(s_k)\in\R^{d_z}$. Right: LLAE predicts complex latents $\hat{z}(s_k)\in\C^{d_z}$ (stored as $2d_z$ reals).}
    \label{fig:direct_surr}
\end{figure}

\subsubsection{SVD Dimensionality Reduction}

To reduce the effective output dimension of the surrogate and simplify its learning task, one can apply a preliminary offline SVD compression to the latent sequence before training the surrogate. Given the latent codes $\{e_i(\vmu)\}_{i=1}^{M}$ collected over the training set (with $M = K$ for SLAE, $M = N_t$ for LLAE), a truncated SVD yields a projection matrix $V \in \R^{d_z \times d_{\mathrm{SVD}}}$ (with $d_{\mathrm{SVD}} \ll d_z$) such that $e_i \approx (e_i V) V^\top$. The reduced codes $G_i(\vmu) = e_i(\vmu)\, V \in \R^{d_{\mathrm{SVD}}}$ become the surrogate targets, reducing the prediction problem from dimension $d_z$ to $d_{\mathrm{SVD}}$.
...
\end{equation}
where $w_k^{\text{pred}}$ and $w_k^{\text{true}}$ denote the predicted and true Laplace-domain latents (either $z(s_k)$ or $\hat{G}(s_k)$ depending on the chosen strategy). End-to-end backpropagation allows gradients from the spatiotemporal MSE to flow through the frozen decoder and the inverse transform, updating the surrogate weights and, in the SVD variant, the projection basis $V$.
...
\textit{Proof Sketch} ($\epsilon_{\mathrm{AE}}(s_k) \leq \mathcal{E}_{\mathrm{SVD}}(s_k)$).
Consider an Autoencoder trained on the spatial fields at frequency $s_k$, with an encoder $f_\phi$ and a decoder $g_\psi$ mapping to a latent dimension $d_z$. The hypothesis space of non-linear autoencoders inherently includes linear transformations. Let $W_{\mathrm{enc}}$ and $W_{\mathrm{dec}}$ represent the optimal linear projection matrices derived from the SVD of the dataset matrix $\bm{U}^{(k)}$. By setting the network weights to these linear matrices and configuring the activation functions to operate in their locally linear regimes (e.g., linear regions of ReLU), the AE can exactly replicate the SVD reconstruction. Since the AE is trained over a superset of linear functions, the optimal non-linear parameters theoretically yield an expected error equal to or strictly less than the optimal linear SVD parameters for that frequency. \hfill$\square$

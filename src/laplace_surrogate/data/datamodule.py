...
class TransientDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if laplace:
            ...
            use_optimal = self.cfg.model.get('optimal_laplace', False)
            opt_path    = self.cfg.model.get('optimal_laplace_path', None)

            if use_optimal:
                if not opt_path:
                    raise ValueError(
                        "optimal_laplace=true mais optimal_laplace_path n'est pas défini."
                    )
                import torch as _torch
                ckpt   = _torch.load(opt_path, map_location='cpu')
                s_opt  = ckpt['s_opt']  # complex tensor (K,)
                s_list = s_opt.numpy().astype(np.complex128)
                print(f"[datamodule] s_list chargés depuis {opt_path}  (K={len(s_list)})")
            else:
